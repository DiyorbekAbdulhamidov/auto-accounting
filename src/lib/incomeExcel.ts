// ============================================================
// INCOME (KIRIM) SVERKASI — Excel hisoboti (5 varaq)
//
...
export function buildIncomeWorkbook(
  ...
  if (years.length > 0) {
    ...
    for (const y of years) {
      ...
      yc += c;
      yf += f;
      styleBody(ws.addRow([y, '', c, f, c - f, verdictText(c - f)]), [3, 4, 5], [1, 2, 6]);
    }
    styleTotal(ws.addRow(['ЖАМИ', '', yc, yf, yc - yf, verdictText(yc - yf)]), [3, 4, 5]);
  }
